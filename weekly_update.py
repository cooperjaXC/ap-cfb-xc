...
regenerate the current-week graphs referenced by the README. If this week's data turns out to be a
season's actual Final AP poll, also regenerate that scoring mode's all-time Final-rankings-by-year
graph - otherwise that graph is left untouched, since it should only move once a season truly ends.
...
def weekly_update():
    ...
    just recorded - a normal in-season or preseason week leaves that graph alone.
    """
