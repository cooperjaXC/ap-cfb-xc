...
def realign_teams(df: pd.DataFrame, n_teams_score: int = 5):
    ...
    for move_to_conf, realigned_teams_list in team_movements.items():
        if realigned_teams_list and (
            isinstance(realigned_teams_list, list)
            or isinstance(realigned_teams_list, tuple)
        ):
            # If the list exists:
            for team_being_realigned in realigned_teams_list:
                ...
                exist_spot_tuple = find_team_conference(
                    existing_conference_dict, team_being_realigned
                )
                ...
                if exist_spot_tuple:
                    ...
                    team_name = team_teamscore_tuple[
                        0
                    ]  # Should duplicate team_being_realigned
                    ...
                    working_conf_dict = move_team_to_new_conf(
                        work_conf_d=working_conf_dict,
                        team_to_move=team_being_realigned,
                        move_to_conf=move_to_conf,
                    )
                    print("- - - - - - - - - - - - - - - -")
    ...
    sorted_conf_dict = {
        conf: teams for conf, teams in sorted_conf_dict.items() if teams
    }
...
if __name__ == "__main__":
    ...
    df_4team = pd.read_csv(
        os.path.join(path_to_your_file, "data/2023/4_team/2023_week_final.csv"),
        header=None,
    )
    df_5team = pd.read_csv(
        os.path.join(path_to_your_file, "data/2023/5_team/2023_week_final.csv"),
        header=None,
    )
    ...
    df_4team_realigned.to_csv(
        os.path.join(
            path_to_your_file, "data/2023/4_team/2023_week_final_4team_realigned.csv"
        ),
        index=False,
        header=False,
    )
    df_5team_realigned.to_csv(
        os.path.join(
            path_to_your_file, "data/2023/5_team/2023_week_final_5team_realigned.csv"
        ),
        index=False,
        header=False,
    )
